...
function Checkout({planPrice, currentPlan}) {
  ...
  const totalPrice = planPrice + rushDeliveryFee;
        ...
        {Object.entries(checkout).map(([key, value]) => (
          value && (
            <li key={key} className={styles.summaryItem}>
              ...
              <div className={styles.textWrapper}><p className={styles.value}>{key == "rushDeliveryFee" ? value + currency.USD : value}</p></div>
            </li>
          )
        ))}
        <li className={styles.summaryItem}>
          <p className={styles.priceTitle}>Total price: </p>
          <div className={styles.textWrapper}><p className={styles.totalPrice}>{totalPrice}{currency.USD}</p></div>
        </li>
      </ul>
    </div>
  );
